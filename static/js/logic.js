d3.json('/data_table').then(function(data1){
    ...
    for (let i=0; i<data1.length; i++){
		...
		let tractID=data1[i][7];
		dropdownList.append("option").text(tractID);
		console.log(tractID);
        };
...
//Adds the ploygons to the map. 
		L.polygon(change_order_of_cords, {color: tractColor}).bindPopup(`<h3>Alameda County Tract ID: ${data1[i][7]}</h3><hr><p>Social Vulnerability Score: ${data1[i][1]}</p>
